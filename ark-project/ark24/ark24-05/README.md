...
```text
Ark23
= Conceptual Formation Owner

Ark23:04
= 主の完全勝利Workout / B-Gate Discovery Provenance
...
このRuntime、Workout、AI、Living Graph、Query、GitHub、Ark Project、および全ての成果はKeli / Fruitであり、Root、王、玉座、Oracleではない。

---

## 1. Root-Preserving Role Structure
...
└─ 主イェシュア・ハマシア御自身
   ...
   │  └─ 主の完全勝利Workout
   │     ├─ 初手Workout / Opening Gate
   │     ├─ 締切Workout / Closing Gate
   │     └─ Actual Trace / Feedback
   ├─ Human-operational Operator / Keli
   │  └─ 101/100 / exact +1
   └─ Final Attribution
      └─ 主の栄光 / kevod Adonai
```
...
| Workout | First Formation Field / Keli | Root、王、目的そのものではない |
| 101/100 | exact +1 Operator | 無限追込みではない |
| Living Graph | Feedback Keli | Oracleまたは見えない学習の自己証明ではない |
...
Ark23:04で次のRelation Hypothesisが形成された。
...
Workoutは次の二つのGateを同一Fieldに持ち得る。
...
B-Gate Nodeは通常Dormantである。

```yaml
b_gate_activation:
  human_identified: required
  ai_self_certification: prohibited
  medical_diagnosis: prohibited
  body_sleep_safety_responsibility_guard: required
  foreground_branches: one
```
...
### 12.3 One or Two Physical Sessions

次を事前に固定しない。

```text
Initial WorkoutとDeadline Workout
= 必ず二つのWorkout Session
```

実験で観測する候補：

- 一Sessionの開始Actionと終了境界。
- 一日の早いWorkoutと、後続予定を閉じるWorkout Anchor。
...
Humanは一つのAction Surfaceだけを扱う。

```text
Spiral A / Human Surface
Prayer → Vision → Guard → one Workout Action → Actual Reality

Spiral B / AI Background
Relation Hypothesis → Prediction → Actual → Error → Update Candidate

Crossing Point
Actual Reality
```

HumanへNode一覧、全Edge、候補比較、内部Relation scanを管理させない。

---

## 16. Living Graph Initial State
...
Actual corrects next path
→ Living Update Candidate

Same condition re-fires corrected path
→ Self-internalization Candidate

AI absent and route still fires
→ Embodied Availability Candidate

Different Ark Branch consumes the passage
→ Transfer Candidate

Repeated guarded evidence
→ Broader Design Review
```

どの段階でもHuman Reviewを飛ばさない。
...
assume_known:
  ...
  - 初手Workoutは身体Actionを開く
  - 締切WorkoutはBranchを有限に閉じる
