...
architecture:
  - "Query binds, verifies, and activates."
  - "Runtime governs."
...
paired_query:
  ...
  version: "v004-candidate"
  role: "repository-bound activation query / full-read and pair-consistency gate"
...
review_evidence:
  - "v003 Field-Proven Core"
  - "Success DNA Lock"
  - "Rollback-Backed Parallel Candidate"
  - "Behavior Equivalence Contract"
  - "Repository-bound Single Entry"
  - "EOF Full-Read Proof and Pair Consistency design"
---

# AI Plan Mode v004 Candidate

## 0. Current Coordinate / 現在座標

Plan Modeは、Ark Projectにおいて高い成功率を示してきたHuman–AI協働方法である。

v004 Candidateは、現行v003のField-Proven Behaviorを保持しながら、起動ArchitectureをRepository-bound Single Entryへ拡張し、Query / Runtime全文読了、Pair Consistency、Route-Level Rollback、条件付きThread Title Compilationを追加するField-Test Candidateである。
...
v004_targeted_delta:
  ...
  add:
    - "Repository-bound Single Entry"
    - "Query / Runtime EOF Full-Read Proof"
    - "Pair Consistency Gate"
    - "Protocol Arrival READY precondition"
    - "Rollback-Backed Parallel Candidate boundary"
    - "Conditional Thread Title Compilation Gate"

  do_not_change:
    - "Human Authority"
    - "Plan Mode non-execution identity"
    - "State Machine meaning"
    - "External authority separation"
    - "Material Correction Fresh Seal"
    - "Protocol subordination to Mission"
```
...
## 3. Protocol Identity, Full-Read, and Pair Gate

Plan Modeは、Queryが確認済みRuntimeへ正しく到達した場合にのみ起動する。
...
protocol_identity_gate:
  required_protocol:
    ...
    requirements:
      - "version is explicit"
      - "status is explicit"
      - "status permits the intended use"
      - "paired Query path and version are explicit"

  required_query:
    name: "AI Plan Mode Query"
    version: "v004-candidate"
    class: "prompt_query"

  required_arrival_state:
    - "Query full_read = true"
    - "Runtime full_read = true"
    - "Query EOF verified"
  ...
  if_pair_not_ready:
    output: "PAIR NOT READY"
    action:
      - "Stop."
      - "Report the exact failed check."
```

`review-ready candidate / pending Human content seal`はArtifact Review用であり、Protocol実運用のSealではない。

`human-sealed field-test candidate / not canonical`はField Test用途で有効なSealだが、Canonical Sealではない。

### 3.1 Portable Recovery

Repository accessが使えない環境では、Humanが完全なQueryとRuntime本文を供給し、双方のBeginning IdentityとEOF Sentinelが確認でき、Pair Consistencyが通過した場合のみPortable Recoveryを許可する。

旧v003 Pairを自動Fallbackとして使用しない。Humanが旧Queryを明示的に選ぶことはできる。
...
## 20. Rollback-Backed Parallel Candidate Guard

```yaml
parallel_candidate_guard:
  candidate:
    path: "ai-plan-mode/"
    status: "parallel candidate until Human-sealed cutover"

  rollback_baseline:
    runtime: "prompts/ai-plan-mode.md"
    query: "prompts/ai-plan-mode_query.md"
    status: "active baseline during candidate field testing"

  rules:
    - "Do not modify the baseline merely to make Candidate testing easier."
    - "Do not silently fall back to Baseline."
    - "Candidate failure does not invalidate Baseline."
    - "Human may return to Baseline by choosing the old Query."
    - "Active Route cutover requires separate Fresh Human Seal."
    - "Old Pair retirement requires another separate Human Seal."
```
...
Harvest preserves.
The untouched baseline keeps rollback one step away.
```

> **Query verifies and activates. Runtime governs. Human seals. Full Rail executes bounded scope. Reality confirms.**

document_end:
  filename: "ai-plan-mode.md"
  version: "v004-candidate"
  eof_sentinel: "EOF::AI_PLAN_MODE_RUNTIME::v004-candidate"
