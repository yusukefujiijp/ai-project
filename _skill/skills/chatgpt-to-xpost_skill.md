...
abbreviation: "C2X"
version: "v001.1"
version_note: "v001.1 adds One New Element Rule based on F5-002 C2X Review to prevent safe but low-value paraphrase drafts."
class: "A-candidate"
...
v001.1 adds One New Element Rule based on F5-002.  
Purpose: prevent safe but low-value paraphrase drafts.
...
```text
A案: Source Respect型 / 120字前後
B案: Reader Question型 / 200字前後
C案: Trade-off型 / 280字前後
D案: 自分の補足を入れる版
E案: Thread化候補
```
...
複数Draftは、長さだけでなく角度でも差別化する。

短・中・長のサイズ違いだけでは不十分である。

各Draftは、以下のように異なる角度を持つ。
...
新要素は、確認済み情報に基づく安全な一般化、または自分の意見として所有された補足に限定する。

各Draftに入れる新要素は、原則1つだけにする。

複数Draftは、長さではなく「角度」で差別化する。

同一主張のサイズ違いは、複数Draftと数えない。

元投稿の言い換えのみのDraftは、Safety Passでも不合格である。

> Paraphrase is safe, but not enough.  
> Add one grounded new element.  
> One draft, one new value.

---

## 11. Hook Retention Rule
...
## 12. Do-Not-Amplify Handling Rule
...
### 4. Draft A: Source Respect型 / 120字前後

元投稿の価値を明確に増幅する短い案。

元投稿へのRespectを示しつつ、元投稿にない新要素を1つだけ足す。

---

### 5. Draft B: Reader Question型 / 200字前後

読者に考えさせる問いを含む案。

問いは未確認情報ではなく、安全な一般化や実践上の観点から作る。

---

### 6. Draft C: Trade-off型 / 280字前後

元投稿の主張から見える緊張・トレードオフを1つ提示する案。

便利さとコスト、性能と消費、設計と実装、速度と品質などの対比を扱う。
...
必ず「自分の運用に落とすなら」「個人的には」「ここから考えると」などで、自分側の補足であることを明示する。
...
- One New Element: yes / no / weak
- Draft角度差: ok / weak / same-claim-size-variant
- 新要素の種類: My Operation / Reader Question / Trade-off / Opinion-Owned / Caution / Angle Shift
- 新要素が要確認情報ではないか: ok / watch
- 長さ: ok / too long
```
...
5. 読者に伝わるか
6. 今この投稿に乗るべきか
7. One New Elementが安全な価値になっているか
```

---

## 16. Minimal Prompt To Use With This Markdown
...
- 元投稿者Respectを維持する
- 自分の補足は元投稿由来と混同しない
- 各Draftは、元投稿に存在しない新要素を1つだけ含める
- 新要素は、要確認情報や未確認事実ではなく、具体的な自分の運用への落とし込み、読者への問い、安全なトレードオフ、自分の意見として所有された視点などに限定する
- 複数Draftは、長さだけでなく角度を変える
- 同一主張のサイズ違いは複数Draftと数えない
- 元投稿の言い換えだけのDraftは、安全でも不合格
- Xで読める長さへ圧縮する
...
## 17. Example: Fable 5 Workflow Post
...
```text
この投稿の「Fable 5は設計に寄せる」という整理はかなり重要。

ここから考えると、AI活用の差はモデル性能そのものより「何を任せないか」の設計に出やすい。

高性能AIほど、使い切るより役割を絞る方が価値が残るのかもしれない。
```
...
One New Elementは、「何を任せないか」という安全な一般化からの別角度である。
...
- 元投稿者Respectがある
- 自分の補足を元投稿由来と混同していない
- 各Draftに、元投稿には存在しない安全な新要素が1つある
- 新要素が要確認情報ではない
- 複数Draftが長さ違いだけでなく角度違いになっている
- 同一主張のサイズ違いを複数Draftとして扱っていない
- 元投稿の言い換えだけで終わっていない
- Xで読める長さになっている
- 複数案があり、Humanが選びやすい
- Risk Checkがある
...
- 元投稿の言い換えだけで終わっている
- A/B/Cが同一主張のサイズ違いになっている
- 新要素が要確認情報になっている
- 新要素が未確認事実の断定になっている
- 新要素を足すためにClaim Groundingを緩めている
- My Add-onを元投稿由来に見せている
- Xに向かない長文になっている
- Draftが1案しかない
- Risk Checkがない
...
6. 要確認情報が混入した箇所
7. 元投稿の言い換えだけになった箇所
8. Draft同士が同一主張のサイズ違いになった箇所
9. One New Elementが危険なClaimになった箇所
10. Humanが毎回削る箇所
11. X反応が悪かった箇所
12. 読者に伝わらなかった箇所
...
安全なだけでは足りない。  
言い換えだけでは価値が薄い。  
各Draftには、元投稿にない接地済みの新要素を1つだけ足す。  
ただし、新要素は未確認事実ではなく、意見・問い・運用落とし込み・安全なトレードオフとして所有する。  
危険なHookは止める。  
Humanが最後に選ぶ。
...
Add one grounded new element.
Different drafts need different angles.
One draft, one new value.
Compress for X.
Human seals.
```
